...
\newpage
# First, we create two general functions that calculate effect size.
# 1) The first function, OR_calc, calculates odds ratio for generalized linear mixed models, based on the beta coefficient estimate (the logit) and the standard error estimate. 
# 2) the Second function, Cohensf_calc, calculates the 
```{r oddsratiofunction}

OR_calc <- function(estimate, stderr) {
  OR_output<-matrix(1,1,3)
  colnames(OR_output)<-c("odds-ratio","95% CI lower", "95% CI upper")
  OR_output[1,1] = exp(estimate)
  OR_output[1,2] = exp(estimate - (1.96*se.group))
  OR_output[1,3] = exp(estimate + (1.96*se.group))
  return(OR_output)
}

# Function to calculate Cohen's f
Cohensf_calc <- function(model_ab, model_a=NULL) {
  require(MuMIn)
  # Proportion of variance accounted for by A and B, relative to model with no regressors and intercepts only
  Rab_sq = r.squaredGLMM(object = model_ab)
  
  if (!is.null(model_a)) {
    # Proportion of variance accounted for by A, relative to model with no regressors and intercepts only
    Ra_sq = r.squaredGLMM(object = model_a)
  } else {
    Ra_sq = 0
  }

  # Calculating Cohen's f based on marginal r squared value (Selya 2012)
  f_sq = (Rab_sq[1] - Ra_sq[1]) / (1-Rab_sq)[1]
  return(f_sq)
}
...
summary(m.RR.money)
m.RR.money.nogroup<-glmer(formula = subRewarded ~ moneyCode + (1+moneyCode|subID),
          data = incentive, family = binomial)
m.RR.money.nomoney<-glmer(formula = subRewarded ~ groupCode + (1+moneyCode|subID),
          data = incentive, family = binomial)
# Calculating cohen's f for age factor
Cohensf_calc(model_ab = m.RR.money, model_a = m.RR.money.nogroup)
# Calculating cohen's f for monetary reward factor
Cohensf_calc(model_ab = m.RR.money, model_a = m.RR.money.nomoney)

# # Extracting the coefficients, and calculating the odds ratios
# coef(summary(m.RR.money))
# # Calculate odds ratio for group factor
# coef.group<-coef(summary(m.RR.money))["groupCode","Estimate"]
# se.group<-coef(summary(m.RR.money))["groupCode","Std. Error"]
# OR_calc(coef.group, se.group)
# # Odds ratio for money factor
# coef.money<-coef(summary(m.RR.money))["moneyCode","Estimate"]
# se.money<-coef(summary(m.RR.money))["moneyCode","Std. Error"]
# OR_calc(coef.money, se.money)
...
# Calculating cohen's f for younger adults
Cohensf_calc(model_ab = m.RR.money.YA)
# # Odds ratio for money factor
# coef.money<-coef(summary(m.RR.money.YA))["moneyCode","Estimate"]
# se.money<-coef(summary(m.RR.money.YA))["moneyCode","Std. Error"]
# OR_calc(coef.money, se.money)
...
# Calculating cohen's f for older adults
Cohensf_calc(model_ab = m.RR.money.OA)
# # Odds ratio for money factor
# coef.money<-coef(summary(m.RR.money.OA))["moneyCode","Estimate"]
# se.money<-coef(summary(m.RR.money.OA))["moneyCode","Std. Error"]
# OR_calc(coef.money, se.money)
```
...
summary(m.RT.money)
m.RT.money.nogroup<-lmer(formula = RT ~ moneyCode + (1+moneyCode|subID),
          data = incentiveRT, REML = FALSE)
m.RT.money.nomoney<-lmer(formula = RT ~ groupCode + (1+moneyCode|subID),
          data = incentiveRT, REML = FALSE)
# Calculating cohen's f for age factor
Cohensf_calc(model_ab = m.RT.money, model_a = m.RT.money.nogroup)
# Calculating cohen's f for monetary reward factor
Cohensf_calc(model_ab = m.RT.money, model_a = m.RT.money.nomoney)

#m.RT.money.Rsquared = 1-var(residuals(m.RT.money))/(var(model.response(model.frame(m.RT.money))))
# anova(m.RT.money)
# # Calculating eta squared for group effect
# SS_factor = anova(m.RT.money)["groupCode","Sum Sq"]
# SS_total = sum(anova(m.RT.money)[,"Sum Sq"])
# eta_squared = SS_factor/SS_total
#test<-mixed(formula = RT ~ moneyCode * groupCode + (1+moneyCode|subID), data = incentiveRT, method = "S")
...
# Calculating cohen's f for younger adults
Cohensf_calc(model_ab = m.RT.money.YA)
...
# Calculating cohen's f for older adults
Cohensf_calc(model_ab = m.RT.money.OA)
```
...
m.ACC.money <-glmer(formula = ACC ~ moneyCode * groupCode + (1+moneyCode|subID),
          data = incentive, family="binomial")
summary(m.ACC.money)
m.ACC.money.nogroup<-glmer(formula = ACC ~ moneyCode + (1+moneyCode|subID),
          data = incentive, family="binomial")
m.ACC.money.nomoney<-glmer(formula = ACC ~ groupCode + (1+moneyCode|subID),
          data = incentive, family="binomial")
# Calculating cohen's f for age factor
Cohensf_calc(model_ab = m.ACC.money, model_a = m.ACC.money.nogroup)
# Calculating cohen's f for monetary reward factor
Cohensf_calc(model_ab = m.ACC.money, model_a = m.ACC.money.nomoney)

# # Calculate odds ratio for group factor
# coef.group<-coef(summary(m.ACC.money))["groupCode","Estimate"]
# se.group<-coef(summary(m.ACC.money))["groupCode","Std. Error"]
# OR_calc(coef.group, se.group)
# # Odds ratio for money factor
# coef.money<-coef(summary(m.ACC.money))["moneyCode","Estimate"]
# se.money<-coef(summary(m.ACC.money))["moneyCode","Std. Error"]
# OR_calc(coef.money, se.money)
...
# Calculating cohen's f for younger adults
Cohensf_calc(model_ab = m.ACC.money.YA)

# # Odds ratio for money factor
# coef.money<-coef(summary(m.ACC.money.YA))["moneyCode","Estimate"]
# se.money<-coef(summary(m.ACC.money.YA))["moneyCode","Std. Error"]
# OR_calc(coef.money, se.money)
...
# Calculating cohen's f for older adults
Cohensf_calc(model_ab = m.ACC.money.OA)

# # Odds ratio for money factor
# coef.money<-coef(summary(m.ACC.money.OA))["moneyCode","Estimate"]
# se.money<-coef(summary(m.ACC.money.OA))["moneyCode","Std. Error"]
# OR_calc(coef.money, se.money)
```
...
m.RR.motblock.OA.b2 <-glmer(formula = subRewarded ~ liqCode + moneyCode + 
                              (1+moneyCode+liqCode|subID),
...
# block 2
m.RR.motblock.YA.b2 <-glmer(formula = subRewarded ~ liqCode + moneyCode + 
                              (1+moneyCode+liqCode|subID),
